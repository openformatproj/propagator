...
def c_updater(location, requirements):
    with open(requirements['A'].location.path, 'r', encoding='utf-8') as reader:
        with open(location.path, 'w', encoding='utf-8') as writer:
...
try:
    ...
    A = prop.Resource(prop.FileLocation(pathlib.Path(f'{this_path}/a.txt')), 'A', prop.void_function, prop.void_function)
    B = prop.Resource(prop.FileLocation(pathlib.Path(f'{this_path}/b.txt')), 'B', prop.void_function, prop.void_function)
    C = prop.Resource(prop.FileLocation(pathlib.Path(f'{this_path}/c.txt')), 'C', prop.void_function, c_updater)
